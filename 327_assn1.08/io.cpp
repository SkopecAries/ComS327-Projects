...
void io_battle(character *aggressor, character *defender)
{
  ...
  mvprintw(0, 0, "Press ESC to exit.\nYou're in a battle with a %s. They have %d Pokemon.\nMain Pokemon: %s\nLevel: %d Gender: %s\n--------\nHP: %d\nATK: %d\nDEF: %d\nSp-ATK: %d\nSp-DEF: %d\nSPEED: %d\n--------\nMoves:\n%s lvl: %d",
  char_type_name[n->ctype], (int)n->char_poke.size(), n->char_poke[0].name.c_str(), n->char_poke[0].level, n->char_poke[0].gender.c_str(), n->char_poke[0].hp, n->char_poke[0].attack, n->char_poke[0].defense, n->char_poke[0].special_attack, n->char_poke[0].special_defense, n->char_poke[0].speed,
  n->char_poke[0].move_name[0].c_str(), n->char_poke[0].poke_moves[0].level);
  if (n->char_poke[0].move_num == 2) {
      mvprintw(14,0, "%s lvl: %d", n->char_poke[0].move_name[1].c_str(), n->char_poke[0].poke_moves[1].level);
  }
  do {
    if (getch() == 27) {
      esc = 1;
...
void io_pokemon() {
  ...
  int esc = 0;
  if (Poke.is_shiny == 1) {
    mvprintw(0, 0, "Press ESC to exit.\nA wild SHINY %s has appeared!\nLevel: %d Gender: %s\n--------\nHP: %d\nATK: %d\nDEF: %d\nSp-ATK: %d\nSp-DEF: %d\nSPEED: %d\n--------\nMoves:\n%s lvl: %d", 
    Poke.name.c_str(), Poke.level, Poke.gender.c_str(), Poke.hp, Poke.attack, Poke.defense, Poke.special_attack, Poke.special_defense, Poke.speed,
    Poke.move_name[0].c_str(), Poke.poke_moves[0].level);
    if (Poke.move_num == 2) {
      mvprintw(13,0, "%s lvl: %d", Poke.move_name[1].c_str(), Poke.poke_moves[1].level);
    }
  } else {
