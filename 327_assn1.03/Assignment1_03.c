...
typedef enum {
    char_pc, // Player Character
    char_hiker, // Hiker
    char_rival, // Rival
    char_count // Total number of character types
} char_type_t;
...
// an array of the movement costs given for the assignment
int movement_costs[char_count][ter_count] = {
    //debug   boulder   tree   path  mart center TgrassSgrass mountain forest water   gate,     pc (path)
    {INT_MAX, INT_MAX, INT_MAX, 10,   10,   10,   20,   10, INT_MAX, INT_MAX, INT_MAX, 10,      10}, // PC
    {INT_MAX, INT_MAX, INT_MAX, 10,   50,   50,   15,   10, 15,      15,      INT_MAX, INT_MAX, 10}, // Hiker
    {INT_MAX, INT_MAX, INT_MAX, 10,   50,   50,   20,   10, INT_MAX, INT_MAX, INT_MAX, INT_MAX, 10}, // Rival
    // INT_MAX represents the infinite game cost
};
...
static void dijkstra_path_char(map_t *m, PC* pc, int8_t charType)
{
  static path_t path[MAP_Y][MAP_X], *p;
  static uint32_t initialized = 0;
  heap_t h;
  ...
  while ((p = heap_remove_min(&h))) {
    ...
    // figure out the ter type and assign the cost to c_cost
    terrain_type_t terrain = mappair(p->pos);

    if (charType == char_hiker) {
        c_cost = movement_costs[char_hiker][terrain];
    } else if (charType == char_rival) {
        c_cost = movement_costs[char_rival][terrain];
    } else if (charType == char_pc) {
        c_cost = movement_costs[char_pc][terrain];
    }

    // getting rid of edge penalty and changing + heightpair to + c_cost (also adding more directions)
    if ((path[p->pos[dim_y] - 1][p->pos[dim_x]    ].hn) &&
        (path[p->pos[dim_y] - 1][p->pos[dim_x]    ].cost >
         (p->cost + c_cost))) {
      path[p->pos[dim_y] - 1][p->pos[dim_x]    ].cost =
...
// finds a valid location for the PC to be (similar to the buildings)
void validPC(int *locations, int *size, map_t *map) {
    *size = 0;
    for (int i = 1; i < MAP_Y - 1; i++)
    {
        for (int j = 1; j < MAP_X - 1; j++)
        {
            if (map->map[i][j] == ter_path)
            {
                locations[*size] = i * MAP_X + j;
                (*size)++;
            }
        }
    }
...
// places the PC
static int placePC(map_t *map, PC *pc) {
    int locations[MAP_X * MAP_Y];
    int size;
    validPC(locations, &size, map);

    // generates position for PC
    int rand1 = rand() % size;
    int position = locations[rand1];
    map->map[position / MAP_X][position % MAP_X] = temp_pc;
    pc->y = position / MAP_X;
    pc->x = position % MAP_X;
...
int main(int argc, char *argv[])
{
  ...
  do {
    ...
    // find best path for character to get to player
    dijkstra_path_char(world.cur_map, player, char_hiker);
    dijkstra_path_char(world.cur_map, player, char_rival);
