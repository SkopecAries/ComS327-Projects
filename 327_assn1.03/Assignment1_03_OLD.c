...
typedef enum {
    TERRAIN_BOULDER, // Boulder
    TERRAIN_TREE, // Tree
    TERRAIN_PATH, // Path
    TERRAIN_PMART, // PokeMart
    TERRAIN_PCNTR, // PokeCenter
    TERRAIN_TGRAS, // Tall Grass
    TERRAIN_SGRAS, // Short Grass
    TERRAIN_WATER, // Water
    TERRAIN_GATE, // Gate
    TERRAIN_COUNT // Total number of terrain types
} TerrainType;
...
typedef enum {
    ...
    CHAR_HIKER, // Hiker
    CHAR_RIVAL, // Rival
    // CHAR_SWIMMER, // Swimmer (for future use)
    // CHAR_OTHER, // Other (for future use)
    CHAR_COUNT // Total number of character types
} CharType;

// an array of the movement costs given for the assignment
int movement_costs[CHAR_COUNT][TERRAIN_COUNT] = {
    //BOULDER TREE    PATH  PMART PCNTR TGRAS SGRAS WATER  GATE
    {INT_MAX, INT_MAX, 10,   10,   10,   20,   10, INT_MAX, 10}, // PC
    {INT_MAX, INT_MAX, 10,   50,   50,   15,   10, INT_MAX, INT_MAX}, // Hiker
    {INT_MAX, INT_MAX, 10,   50,   50,   20,   10, INT_MAX, INT_MAX}, // Rival
...
void validPC(int *locations, int *size, Map *map) {
    *size = 0;
    for (int i = 1; i < M_HEIGHT - 1; i++)
    {
        for (int j = 1; j < M_WIDTH - 1; j++)
        {
            if (map->tiles[i][j] == PATH)
            {
                locations[*size] = i * M_WIDTH + j;
                (*size)++;
            }
...
// places the PC (similar to the buildings)
void placePC(Map *map, PC *pc) {
    int locations[M_HEIGHT * M_WIDTH];
    int size;
    ...
    int rand1 = rand() % size;
    int position = locations[rand1];
    map->tiles[position / M_WIDTH][position % M_WIDTH] = Player;
    pc->x = position / M_WIDTH;
    pc->y = position % M_WIDTH;
}
