...
void new_hiker()
{
  ...
  poke Poke = poke();
  c->char_poke.push_back(Poke);

  for (int i = 1; i < 6; i++) {
      if (rand() % 10 < 6) {
          poke PokeNext = poke();
          c->char_poke.push_back(PokeNext);
      } else {
          break;
      }
  }
...
void game_loop()
{
  ...
  while(!done) {
    clear();
    mvprintw(0,0,"Please select your starter Pokemon.\nPress 1 for Pokemon 1, 2 for Pokemon 2, and 3 for Pokemon 3.");
    mvprintw(2,0,"------\nPokemon 1: %s\nGender: %s\nHP: %d ATK: %d DEF: %d Sp-ATK: %d Sp-DEF: %d SPEED: %d\nMoves:\n%s lvl: %d",
    Poke1.name.c_str(), Poke1.gender.c_str(), Poke1.hp, Poke1.attack, Poke1.defense, Poke1.special_attack, Poke1.special_defense, Poke1.speed,
    Poke1.move_name[0].c_str(), Poke1.poke_moves[0].level);
    if (Poke1.move_num == 2) {
      mvprintw(8,0, "%s lvl: %d", Poke1.move_name[1].c_str(), Poke1.poke_moves[1].level);
    }
    mvprintw(9,0,"-------\nPokemon 2: %s\nGender: %s\nHP: %d ATK: %d DEF: %d Sp-ATK: %d Sp-DEF: %d SPEED: %d\nMoves:\n%s lvl: %d",
    Poke2.name.c_str(), Poke2.gender.c_str(), Poke2.hp, Poke2.attack, Poke2.defense, Poke2.special_attack, Poke2.special_defense, Poke2.speed,
    Poke2.move_name[0].c_str(), Poke2.poke_moves[0].level);
    if (Poke2.move_num == 2) {
      mvprintw(15,0, "%s lvl: %d", Poke2.move_name[1].c_str(), Poke2.poke_moves[1].level);
    }
    mvprintw(16,0,"------\nPokemon 3: %s\nGender: %s\nHP: %d ATK: %d DEF: %d Sp-ATK: %d Sp-DEF: %d SPEED: %d\nMoves:\n%s lvl: %d",
    Poke3.name.c_str(), Poke3.gender.c_str(), Poke3.hp, Poke3.attack, Poke3.defense, Poke3.special_attack, Poke3.special_defense, Poke3.speed,
    Poke3.move_name[0].c_str(), Poke3.poke_moves[0].level);
    if (Poke3.move_num == 2) {
      mvprintw(22,0, "%s lvl: %d", Poke3.move_name[1].c_str(), Poke3.poke_moves[1].level);
    }
    // just exit for now since it doesn't need saved for this assignment
    switch(getch()) {
      case '1':
        world.pc.char_poke.push_back(Poke1);
        done = 1;
        break;
      case '2':
        world.pc.char_poke.push_back(Poke2);
        done = 1;
        break;
      case '3':
        world.pc.char_poke.push_back(Poke3);
        done = 1;
        break;
      default:
        break;
